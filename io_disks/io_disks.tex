...
\itms{
\item Position specified by <Cyl., Head, Sector>
\item Each track is specified by the <Cyl.,Head> tuple
...
\begin{slide}{Transfer Time}
    $Transfer~Time = Seek~Time + Rotational~Delay + \frac{Data 
    Size}{MIN(Bus~Speed, Max~Read~Speed)}$
...
\begin{frame}
\frametitle{Sectors -- LBA Addressing}
\itms{
\item Modern disk interfaces present linear array of \emph{sectors}
	\ittms{
...
\item Disk maps logical sector \#s to physical sectors
	\ittms{
	  ...
	  \item \textit{Track skewing}--sector 0 pos.\ varies by track for 
	      sequential accesses
...
\begin{slide}{Disk controller interface}
\itms{
\item Disks connected by bus (e.g., IDE/ATA, SATA, SCSI, SAS)
\ittms{
    \item Multiple devices may contend for bus
}
\item \Red{Advanced interfaces support:}
%\item Disconnect from bus during requests %(+200 $\mu$s SCSI)
\item {\em Command queuing}: Give disk multiple requests
	\ittms{
	\item Disk can schedule them using rotational information
	\item OS can limit IO reordering
	}
\item Disk cache used for {\em read-ahead}
	\ittms{
	\item Otherwise, sequential reads would incur whole revolution
	\item Cross track boundaries?  Can't stop a head-switch
	}
\item {\em Write caching}
	\ittms{
