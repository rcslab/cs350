...
\itms{
  ...
  \ittms{
    \item Sets \textit{close on exec} flag if \texttt{val} = 1,
	clears if \texttt{val} = 0
    \item Sets file descriptor non-inheritable by new program
  }
...
\vspace*{-1mm}
  ...
  \ittms{
    ...
    \item Called \texttt{proc} in Unix, \texttt{task\_struct} in
      Linux, and just \texttt{struct thread} in OS/161
  }
...
\ittms{
  ...
  (OS/161 \texttt{kern/thread/thread.c})
}
\item Priority?
\ittms{
