...
%\newcommand{\map}[3]%
%  {{\def\in{#1}\def\map{#2}\def\out{#3}\input{map}}}
...
%\def\cents{\hbox{\rm\rlap{\hspace*{.45ex}\vrule height 1.2ex depth .25ex}c}}
\makeatletter
\DeclareRobustCommand{\cents}%
  {\def\bx{bx}\def\f/{\f@family\ifx\f@series\bx b\else r\fi}%
      \font\rawpal=\f/ at\f@size pt\rawpal\char162}
\makeatother
% Alternatively
%\usepackage[TS1,T1]{fontenc}
%\def{\cents}{\UseTextSymbol{TS1}{\textcent}}
...
\end{tikzpicture}
    ...
    \item File system's job: translate name \& offset to disk blocks:
      \\[.5ex]
      \map{\{file, offset\}}{FS}{disk address}
  }
...
\itms{
  ...
  \item Next lecture:
  \ittms{
    \item FFS in more detail
    \item More advanced, modern file systems
  }
}
\end{slide}
...
%% \begin{slide}{PSA}
%% \vspace*{.5in}
%% \centerline{\includegraphics[width=4in]{vote}}
%% \end{slide}
