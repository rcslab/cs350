...
\section{How to implement threads in OS/161}

\begin{slide}{Background: MIPS calling conventions}
  \vspace{-1em}
  \begin{columns}
  \column{.65\textwidth}   
  \itms{
    ...
    \ittms{
      \item Functions free to clobber \Red{\emph{caller-saved}}
        \rlap{regs} \\
        (\texttt{\%t0}--\texttt{\%t9} on \rlap{MIPS)}
      \item But must restore \Red{\emph{callee-saved}} ones to
        original value upon return (\texttt{\%s0}--\texttt{\%s7}, \texttt{\%fp})
    }
    \item \texttt{\%sp} register always base of stack
    \ittms{
      \item Frame pointer (\texttt{\%fp}) is old \texttt{\%sp}
    }
    \item Local variables stored in registers and on stack
    \item Function arguments go in caller-saved regs and on stack
    \ittms{
      \item First four arguments in \texttt{\%a0}--\texttt{\%a3}
      \item Remaining arguments on stack
    }
    \item Return value \texttt{\%v0} and \texttt{\%v1}
  }
  \column{.05\textwidth}
  \column{.3\textwidth}
  \input{stkframe.tex}
...
\begin{slide}{OS/161 Kernel Threads}
  \itms{
  ...
  \itms{
    \item OS/161 supports kernel threads (no user-level threading)
    \item Create a kernel thread with: \texttt{thread\_fork()}
    \item Bad nameing: Not \texttt{fork()} this is actually pthread\_create!
  }
\end{slide}


\begin{slide}{Switching Threads}
  \itms{
    \item All thread switches go through \texttt{thread\_yield()} and \texttt{thread\_switch()}
    \item \texttt{thread\_switch()} calls \texttt{switchframe\_switch} generates \texttt{switchframe}
    \item \texttt{switchframe\_switch switches from one stack to other}
    \gap
  }
...
\begin{columns}
  \column{0.5\textwidth}
  \centering
  \begin{drawstack}[every node/.style={scale=0.5},scale=0.5]
    \cell{thread\_yield()}
    \cell{thread\_switch()}
    \cell{switchframe}
  \end{drawstack}
