...
\itms{
  ...
  \ittms{
    ...
    \item \texttt{pthread\_exit()} -- Destroy the current thread
    \item \texttt{pthread\_join()} -- Waits for a thread to exit
  }
}
\end{slide}
...
\itms{
  ...
  \item Thread packages typically provide \emph{mutexes}: \\
    \code{void mutex_init (mutex_t *m, \ldots);} \\
...
\item \code{int pthread_mutex_init(pthread_mutex_t *m,} \\
    \hspace{12.7em}\code{pthread_mutexattr_t attr)}
\ittms{
    \item Initialize a mutex
}
\item \code{int pthread_mutex_destroy(pthread_mutex_t *m)}
\ittms{
    \item Destroy a mutex
}
\item \code{int pthread_mutex_lock(pthread_mutex_t *m)}
\ittms{
    \item Acquire a mutex
}
\item \code{int pthread_mutex_unlock(pthread_mutex_t *m)}
\ittms{
    \item Release a mutex
}
\item \code{int pthread_mutex_trylock(pthread_mutex_t *m)}
\ittms{
...
\itms{
  ...
  \item \code{int pthread_cond_init(pthread_cond_t *, \ldots);}
  \ittms{
    \item Initialize with specific attributes
  }
  \item \code{int pthread_cond_wait(pthread_cond_t *c, pthread_mutex_t *m);}
  \ittms{
    \item Atomically unlock \texttt{m} and sleep until \texttt{c} signaled
    \item Then re-acquire \texttt{m} and resume executing
  }
  \item \code{int pthread_cond_signal(pthread_cond_t *c);} \\
	\code{int pthread_cond_broadcast(pthread_cond_t *c);}
  \ittms{
...
\begin{slide}{Monitors
    [BH]\cref{readings/monitors.pdf}{[Hoar]}}
\itms{
...
\itms{
  ...
  \ittms{
    \item \code{sem_wait(sem_t *s)}\quad  (originally called $P$)
    \item \code{sem_signal(sem_t *s)}\quad (\code{sem_post} in PThreads, 
	originally called $V$)
  }
  \item Operation:
    \texttt{sem\_wait} will return only $N$ more times than 
